...
class App(ctk.CTk):
    def __init__(self):
        ...
        self.confirm_button = ctk.CTkButton(self.input_frame, text="Confirm CPG", command=self.confirm_cpg)
        self.confirm_button.pack()

    def confirm_cpg(self):
        selected_cpg = self.cpg_dropdown.get()
        self.chosen_cpg = self.data[selected_cpg]
        print(self.chosen_cpg)
    ...
    def build_gui(self):
        ...
        diseases = list(set(treatment['disease'] for treatment in self.chosen_cpg['_default'].values()))
        self.disease_var = tk.StringVar()
        self.diseases_label = ctk.CTkLabel(self.input_frame, text="Select disease:")
        self.diseases_label.grid(row=21, column=0, padx=10, pady=5)
        self.disease_dropdown = ctk.CTkOptionMenu(self.input_frame, variable=self.disease_var, values=diseases)
        ...
        self.disease_var.trace_add("write", self.update_severity_dropdown)
    ...
    def update_severity_dropdown(self, *args):
        ...
        for treatment in self.chosen_cpg['_default'].values():
            if treatment['disease'] == selected_disease:
                for eligibility in treatment['eligibility']:
                    for key, value in eligibility['severity'].items():
                        severity = f"{key}: {value.strip()}"  
                        unique_severities.add(severity)

        severities_list = sorted(list(unique_severities))
        print(severities_list)

        if hasattr(self, 'severity_dropdown'):
            self.severity_dropdown.destroy()

        self.severity_dropdown = ctk.CTkOptionMenu(self.input_frame, variable=self.severity_var, values=severities_list)
        self.severity_dropdown.grid(row=22, column=1, padx=10, pady=5)

        if severities_list:
            self.severity_var.set(severities_list[0])
        else:
    ...
    def retrieve_treatments(self):
        ...
        for disease_info in user_diseases:
            ...
            treatments = self.chosen_cpg['_default'].values()
